...
namespace ColdDesertNights.Utility
{
    public static class SpacerDrawer
    {
        ...
        private static bool Draw(string label, Rect controlRect)
        {
            ...
            var color = GUI.color;
            GUI.color = LineColor;
            Widgets.DrawLineHorizontal(left, (controlRect.height / 2) + controlRect.y,
                (controlRect.width * 2) - textWidth);
            GUI.color = color;
            return false;
        }
        ...
        public static void GenerateSpacer(string label, ModSettingsPack settings,
